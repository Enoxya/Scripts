...
#    [parameter(Position=0,Mandatory=$true,ValueFromPipeline=$false,HelpMessage='Filename to write HTML report to')][string]$HTMLReport,
	[parameter(Position=1,Mandatory=$false,ValueFromPipeline=$false,HelpMessage='Send Mail ($True/$False)')][bool]$SendMail=$true,
...
$moisAnnee = Get-Date -Format 'MMMM yyyy'
$mois_Annee = Get-Date -Format 'MMMM_yyyy'
$MoisAnnee = (Get-Culture).textinfo.totitlecase($moisAnnee)
$Mois_Annee = (Get-Culture).textinfo.totitlecase($mois_Annee)
$NomRapport = "Exchange_Rapport_"+$Mois_Annee+".html"

#A utiliser si envoi du rapport par mail :
$Emetteur = "Exchange@autobernard.com"
$Destinataires = @("dsiinfra@autobernard.com", "s.saunier@autobernard.com", "n.may@autobernard.com", "w.esilva@autobernard.com", "x.strottner@autobernard.com") 
$SujetMessage = "Exchange - Environnement - Rapport mensuel - $MoisAnnee"
$ServeurMessagerie = "exch-relais.groupe-bernard.lan"
...
<h2 align=""center"">Exchange - Environnement - Rapport - $($MoisAnnee)</h3>
<h4 align=""center"">Date de génération : $((Get-Date).ToString())</h5>
</font>
...
$Output+="</body></html>";
$Output | Out-File $NomRapport


if ($SendMail)
{
	_UpProg1 95 "Sending mail message.." 4
	Send-MailMessage -Attachments $NomRapport -From $Emetteur -To $Destinataires -Subject $SujetMessage -BodyAsHtml $Output -SmtpServer $ServeurMessagerie
}